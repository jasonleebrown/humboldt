...
humboldt.occ.rarefy <- function(in.pts, colxy = 2:3, rarefy.dist = 0, rarefy.units = "km", run.silent.rar = F) {
    ...
    repeat {
        nn1 <- nndist.spatstat.legacy(del.min.dist[, "x4r"], del.min.dist[, "y4r"])  # calculate distance nearest neighbour
        ...
        # nearest neighbour
        nn2 <- nndist.spatstat.legacy(del.min.dist[, "x4r"], del.min.dist[, "y4r"], k = 2)
        ...
        if (sum(del2) > 1) {
            for (k in 3:6) {
                nn <- nndist.spatstat.legacy(del.min.dist[, "x4r"], del.min.dist[, "y4r"], k = k)
...
nndist.spatstat.legacy <- function(X, ...) {
  UseMethod("nndist")
}
