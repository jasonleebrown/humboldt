...
humboldt.background.stat<- function(g2e, rep = 100, sim.dir = 1, env.reso, kern.smooth = 1, R = 100,
	...
	if (force.equal.sample==T){
		...
		if (ncores > 1){
			...
			sim.o <- as.data.frame(matrix(unlist(pbapply::pblapply(1:rep, background.iter.e, z1, scores.env12a, scores.env2a, scores.sp2a, scores.env2c, Rana.x, Rana.y, env.reso.sim, run.silent.bak, inE, Rin, ncores, userOS, threshinZ, kern.smoothinZ, pb, repT, cl=cl)), byrow = TRUE, ncol = 9)) #simulate random overlap
    ...
    if (force.equal.sample==F){
		...
		if (ncores > 1){
			cl <- snow::makeCluster(ncores)  #open a cluster for parallelization
			snow::clusterExport(cl=cl, c("z1", "scores.env12a", "scores.env2a", "scores.sp2a", "scores.env2c", "Rana.x", "Rana.y"), envir=environment())
			invisible(snow::clusterEvalQ(cl, library(humboldt)))
			sim.o <- as.data.frame(matrix(unlist(pbapply::pblapply(1:rep, background.iter.r, z1, scores.env12a, scores.env2a, scores.sp2a, scores.env2c, Rana.x, Rana.y, env.reso.sim, run.silent.bak, inE, Rin, ncores, userOS, threshinZ, kern.smoothinZ, pb, repT, cl=cl)), byrow = TRUE, ncol = 9)) #simulate random overlap
			snow::stopCluster(cl)  #shutdown the cluster
		}		
		if (ncores == 1 & run.silent.bak == F){close(pb)}
      }
