...
for (i in 1:k.step.num){
    if (is.null(k.eval)){k.eval<-k.min}
	else k.eval<- c(k.eval,k.eval[i-1]+k.step)}

for (i in 1:k.step.num){
	## run create a grid of Environmental Space Function
	z1<- humboldt.grid.espace(scores.env12,scores.env1,scores.sp1,kern.smooth=k.eval[i],R=100)
	z2<- humboldt.grid.espace(scores.env12,scores.env2,scores.sp2,kern.smooth=k.eval[i],R=100)
	#'
	## plot niche in espace
	par(mfrow= c(1,2))
	humboldt.plot.niche(z1,paste("Species 1: Smoothing K=",k.eval[i]),"PC1","PC2")
	humboldt.plot.niche(z2,paste("Species 2: Smoothing K=",k.eval[i]),"PC1","PC2")
	
	val.k[i]<-humboldt.niche.similarity(z1,z2,correct.env=F)
}
print("*************************************************")
print("*************************************************")
