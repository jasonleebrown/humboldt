##tcltk, pbapply, terra, ade4,sp,
## RELEASE CANDIDATE 2.0
################################################################################################
## fixe schedule- second ENV plot function, gbm.step fucntion from dismo
## fixed 8/27/2025
##################################################################################################
...
#' environmental variables to be included in the PCA. This way all included variables are known to be relevant in both species distributions. This can be done inside Humboldt (via humboldt.top.env.brt or humboldt.top.env.maxent) by importing many environmental variables and letting program select only those important. Alternatively, this can also be done using another method (MaxEnt) outside of R and then use only variables deemed important in the species' distributions. \cr \cr 
#' @seealso \code{humboldt.equivalence.stat, humboldt.background.stat, humboldt.niche.similarity, humboldt.plot.niche,humboldt.doitall, humboldt.top.env.brt} that use or depend on outputs of this function 
#' @import sp
#' @import terra
...
humboldt.grid.espace <- function(glob.g, glob.s, sp, R = 100, kern.smooth = 1){
  ...
  if (ncol(glob.g) == 2) {
    ...
    ymax <- max(glob.g[, 2] + Kern.SmZZ)
    ext <- c(xmin, xmax, ymin, ymax) 
	
	#FOR SPECIES
	 xr <- data.frame(cbind(
    (sp[, 1] - ext[1]) / abs(ext[2] - ext[1]),
    (sp[, 2] - ext[3]) / abs(ext[4] - ext[3])
     )) # data preparation
   ...
   sp.dens <- adehabitatHR::kernelUD(sp::SpatialPoints(xr[, 1:2]),
                                       h = Kern.SmZ, grid = mask,
                                       kern = "bivnorm"
   ) # calculate the density of occurrences in a grid of RxR pixels along the score gradients
      sp.dens <- terra::rast(
        matrix(sp.dens$ud,nrow = R)
      )
      terra::ext(sp.dens)<-c(
        xmin = ext[1], 
        xmax = ext[2], 
      ...
      if (!is.null(th.sp)) {
        th.value <- quantile(terra::extract(sp.dens, sp)[,1], th.sp)
        sp.dens[sp.dens < th.value] <- 0
      }
	#FOR HABITAT

    xrg <- data.frame(cbind(
    (glob.s[, 1] - ext[1]) / abs(ext[2] - ext[1]),
    (glob.s[, 2] - ext[3]) / abs(ext[4] - ext[3])
      )) # data preparation
    mask <- adehabitatMA::ascgen(sp::SpatialPoints(cbind((0:(R)) / R, (0:(R) / R))),
                                   nrcol = R - 2, count = FALSE
      ) # data preparation
   
      glob.s.dens <- adehabitatHR::kernelUD(sp::SpatialPoints(xrg[, 1:2]),
                                       h = Kern.SmZ, grid = mask,
                                       kern = "bivnorm"
   ) # calculate the density of occurrences in a grid of RxR pixels along the score gradients
      glob.s.dens <- terra::rast(
        matrix(glob.s.dens$ud,nrow = R)
      )
      terra::ext(glob.s.dens)<-c(
        xmin = ext[1], 
        xmax = ext[2], 
        ymin = ext[3],
        ymax = ext[4]
      ...
      if (!is.null(th.env)) {
        th.value <- quantile(terra::extract(glob.s.dens, glob.s)[,1], th.env)
        glob.s.dens[glob.s.dens < th.value] <- 0
      }
 
    x <- seq(from = ext[1], to = ext[2], length.out = R)
    y <- seq(from = ext[3], to = ext[4], length.out = R)
    l$y <- y
    Z <- glob.s.dens * nrow(glob.s) / terra::global(glob.s.dens, "sum")$sum # rescale density to frequency of  env, ie. number of occurrence/pixel
    ...
    z[is.nan(z)] <- 0 # remove n/0 situations    z
    Z[is.nan(Z)] <- 0 # remove n/0 situations    z
	z.cor <- z/Z # correct for environment prevalence
    z.cor[is.nan(z.cor)] <- 0 # remove n/0 situations
    z.cor[is.na(z.cor)] <- 0 # remove n/0 situations
	z.cor[is.infinite(z.cor)] <- 0
    z.cora <- z.cor / terra::global(z.cor, "max")$max
	crs(Z) <- crs("+proj=longlat +datum=WGS84")
	crs(z) <- crs("+proj=longlat +datum=WGS84")
	crs(z.uncor) <- crs("+proj=longlat +datum=WGS84")
	crs(z.cora) <- crs("+proj=longlat +datum=WGS84")
  }
  ...
  #l$z <- matrix(z,nrow = R)
  l$z <- z
  l$z.uncor <- matrix(z.uncor,nrow = R)
  l$z.cor <- matrix(z.cora,nrow = R)
  l$Z <- matrix(Z,nrow = R)
  #l$Z <- Z
  l$glob.g <- glob.g
  l$glob.s <- glob.s
...
#' @seealso \code{humboldt.equivalence.stat, humboldt.background.stat, humboldt.niche.similarity, humboldt.plot.niche,humboldt.doitall, humboldt.top.env.maxent} that use or depend on outputs of this function 
#' @import sp
#' @import terra
#' @importFrom adehabitatMA ascgen
#' @importFrom adehabitatHR kernelUD
...
#' occ.sp2<-na.exclude(read.delim("sp2.txt",h=T,sep="\t"))
#' 
#' ##its highly recommened that you using the function "humboldt.top.env.maxent" or "humboldt.top.env.brt" to select only the important enviromnetal variables. This step can be skipped. If you downloaded tons of environmental data, you should use this step.  If you skip this step, input env1/env2 inplace of reduc.vars$env1/reduc.vars$env2 
#' reduc.vars<- humboldt.top.env.brt(env1=env1,env2=env2,sp1=occ.sp1,sp2=occ.sp2,rarefy.dist=40, rarefy.units="km",env.reso=0.0833338,e.var=(3:21),pa.nu=10000,method="contrib",contrib.greater=5)
#'
#' 
...
humboldt.pnt.index <- function(glob.g, glob.s, sp, R = 100, kern.smooth = 1) {
    ...
    if (ncol(glob.g) == 1) 
        stop("cannot calculate overlap on one axes")
    if (nrow(sp) < 5) stop("Too few occurrence points")
	if (any(is.na(glob.g))) stop("Missing values in glob.g")
    if (ncol(glob.g) == 2) {
        # if scores in two dimensions (e.g. PCA)
        xmin <- min(glob.g[, 1] - Kern.SmZZ*10)
        xmax <- max(glob.g[, 1] + Kern.SmZZ*10)
        ymin <- min(glob.g[, 2] - Kern.SmZZ*10)
        ymax <- max(glob.g[, 2] + Kern.SmZZ*10)
		ext <- c(xmin, xmax, ymin, ymax) 		# data preparation\t
        ...
        spr <- data.frame(cbind((sp[, 1] - xmin)/abs(xmax - xmin), (sp[, 2] - ymin)/abs(ymax - 
            ymin)))
        mask <- adehabitatMA::ascgen(sp::SpatialPoints(cbind((1:R)/R, (1:R)/R)), nrcol = R - 2, count = FALSE)
        sp.dens <- adehabitatHR::kernelUD(sp::SpatialPoints(spr[, 1:2]), h = Kern.SmZ, grid = mask, kern = "bivnorm")
		sp.dens <- terra::rast(matrix(sp.dens$ud,nrow = R))
        terra::ext(sp.dens)<-c(
        xmin = ext[1], 
        xmax = ext[2], 
        ymin = ext[3],
        ...
        #    nrow = R))
        glob.s.dens <- adehabitatHR::kernelUD(sp::SpatialPoints(glob.sr[, 1:2]), grid = mask, kern = "bivnorm")
		glob.s.dens <- terra::rast(matrix(glob.s.dens$ud,nrow = R))
        terra::ext(glob.s.dens)<-c(
        xmin = ext[1], 
        xmax = ext[2], 
        ymin = ext[3],
        ...
        glob.sr <- terra::extract(glob.s.dens, glob.s)
        Z.th <- quantile(glob.sr$lyr.1, th.env)
        glob.s.dens[glob.s.dens < Z.th] <- 0
		
		denom <- terra::global(sp.dens, "sum")$sum
		if (denom == 0) stop("Kernel density sum is zero")
		Z <- glob.s.dens * nrow(glob.s) / denom# rescale density to frequency of  env, ie. number of occurrence/pixel
        #Z <- glob.s.dens * nrow(glob.s)/cellStats(glob.s.dens, "sum")
        spr <- terra::extract(sp.dens, sp)
        z.th <- quantile(spr$lyr.1, th.sp)
        sp.dens[Z == 0] <- 0
        ...
        z.cor <- z/Z
        z.cor[is.na(z.cor)] <- 0
        z.cor <- z.cor/ terra::global(z.cor, "max")$max  # rescale density to the 
        ...
        for (i in 1:length(Benv)) {		
           ...
           By<-Benv[[i]]$y
           xyB<-cbind(Bx,By)
		   ps1 <- terra::vect(xyB, "polygons")
           if (i ==1) {
				sBenv <- ps1
				}
           if (i !=1) {
				sBenv2 <- ps1
				sBenv <-terra::union(sBenv,sBenv2)
				}           
		   }
        ...
        for (i in 1:length(Bsp)) {
           ...
           Ay<-Bsp[[i]]$y
           xyA<-cbind(Ax,Ay)
		   ps2 <- terra::vect(xyA, "polygons")
           if (i ==1) {
				sBsp <- ps2
				}
           if (i !=1) {
		   		sBsp2 <- ps2
				sBsp <-terra::union(sBsp,sBsp2)
				}
        }
        sBf=sBsp+sBenv
		sBsD <- try(sBf-sBenv, silent = TRUE)
			if (inherits(sBsD, 'try-error')) {area.red<-0}
		ratio<- try({sBsD1 <- terra::as.lines(sBsD)
			sBsD2 <- sf::st_as_sf(sBsD1)		 
			sBsD_l <- sf::st_length(sBsD2)
			sBsp1 <- terra::as.lines(sBsp)
			sBsp2 <- sf::st_as_sf(sBsp1)		 
			sBsp_l <- sf::st_length(sBsp2)
			area.red<-(0.5*sum(sBsD_l))/sum(sBsp_l)			
			}	)
			if (inherits(ratio, 'try-error')) {ratio<-0}
...
humboldt.niche.similarity <- function(z1, z2, correct.env = T, nae = "NO", thresh.espace.z = 0.001) {
    R <- length(z1$x)
    l <- list()
    ...
    if (nae == "NO" & correct.env == F) {
		...
		z1$z.uncor[matrix(za.sum.mask, nrow= R) == 0] <- 0
        #z2$z.uncor[za.sum.mask == 0] <- 0
		z2$z.uncor[matrix(za.sum.mask, nrow= R) == 0] <- 0
        Clc <- round(100 * (za.sum.cf/za.sum.ci), 2)
    }
    if (nae == "NO" & correct.env == T) {
        #z1$z.cor[za.sum.mask == 0] <- 0
		z1$z.cor[matrix(za.sum.mask, nrow= R) == 0] <- 0
        #z2$z.cor[za.sum.mask == 0] <- 0
		z2$z.cor[matrix(za.sum.mask, nrow= R) == 0] <- 0
        Clc <- round(100 * (za.sum.cf/za.sum.ci), 2)
    }
    if (nae == "YES") {
        Clc <- 100
    ...
    if (correct.env == T) {
		...
		z1$z.cor[is.nan(z1$z.cor)] <- 0
        z2$z.cor[z2$z.cor < thresh.espace.z] <- 0
		z2$z.cor[is.nan(z2$z.cor)] <- 0
    ...
    mask.analog.e.raster <- terra::rast(matrix(za.sum.mask, nrow = R))
    #crs(mask.analog.e.raster) <- crs(z1$Z.raster)
    terra::ext(mask.analog.e.raster) <- terra::ext(z1$z)
    l$mask.analog.e.raster <- mask.analog.e.raster
    mask.total.e.raster <- terra::rast(matrix(za.sum, nrow = R))
    #crs(mask.total.e.raster) <- crs(z1$Z.raster)
    terra::ext(mask.total.e.raster) <- terra::ext(z1$z)
    l$mask.total.e.raster <- mask.total.e.raster
    z1.raster <- terra::rast(matrix(p1, nrow = R))
    #crs(z1.raster) <- crs(z1$Z.raster)
    terra::ext(z1.raster) <- terra::ext(z1$z)
    l$z1.raster <- z1.raster
    z2.raster <- terra::rast(matrix(p2, nrow = R))
    #crs(z2.raster) <- crs(z1$Z.raster)
    terra::ext(z2.raster) <- terra::ext(z1$z)
...
humboldt.equivalence.stat <- function(z1, z2, rep = 100, correct.env = T, kern.smooth = 1, nae = "YES", 
	...
	if (kern.smooth == "Auto"){kern.smooth = "auto"}
    threshinZ <- thresh.espace.z; kern.smoothinZ <- kern.smooth; repT=rep; nacinZ <- nae; Rin <- length(z1$x); cor.envn<-correct.env; pb<-NULL
    switch(Sys.info()[['sysname']],
    Windows= {userOS=1},
    Linux  = {userOS=2},
    Darwin = {userOS=2})
	#deal with people who specify too many or all cores
	tCores<-parallel::detectCores()
	if(ncores=="ALL"){ncores<-(tCores-1)}
	if(tCores==ncores & tCores>1){ncores<-(tCores-1)}
    if(tCores<ncores){ncores<-(tCores-1)}
	
	l <- list()
	obs.o <- humboldt::humboldt.niche.similarity(z1, z2, correct.env= cor.envn, nae = nacinZ, thresh.espace.z = threshinZ)  #observed niche similarity
	if (ncores == 1){
		...
		sim.o <- as.data.frame(matrix(unlist(lapply(1:rep, equivalency.iter, z1_sp=z1$sp, z1_glob=z1$glob.g,z2_sp=z2$sp, z2_glob=z2$glob.g, nacinZ, threshinZ, kern.smoothinZ, Rin,pb, repT, ncores, run.silent.equ, cor.envn,userOS)), byrow = TRUE, ncol = 2))}  #simulate random overlap
	...
	progressr::with_progress({
		p <- progressr::progressor(steps = rep)
		
		sim_list <- future.apply::future_lapply(
		  seq_len(rep),
		  function(repi, ...) {
			tryCatch({
				out <- equivalency.iter(repi, ...)
				p()
				out
			}, error = function(e) {
				message("Iteration ", repi, " failed: ", e$message)
				c(NA, NA)
			})
		  },
		  z1_sp = z1_sp,
		  ...
		  nacinZ = nacinZ,
		  threshinZ = threshinZ,
		  kern.smoothinZ = kern.smoothinZ,
		  Rin = Rin,
		  cor.envn = cor.envn,
		  future.seed = TRUE,
		  ncores= ncores,
		  run.silent.equ =run.silent.equ,
		  userOS = userOS
		)
	})

	sim.o <- as.data.frame(do.call(rbind, sim_list))
...
##################################################################################################
## fixed 8/25/2025 - replace placed text
#' internal fuction 
#' @keywords internal
#' @export

equivalency.iter<- function(repi,z1_sp, z1_glob, z2_sp, z2_glob, nacinZ, threshinZ, kern.smoothinZ, Rin, pb, repT, ncores, run.silent.equ, cor.envn, userOS) {
	all.points <- rbind(z1_sp, z2_sp)
    ...
    coordinates.sim1 <- coordinates.sim[1:nrow(z1_sp), ]
    coordinates.sim2 <- coordinates.sim[(nrow(z1_sp) + 1):(nrow(z1_sp) + nrow(z2_sp)), ]
    #z1.sim <- humboldt.grid.espace(z1$glob.g, z1$glob.s, data.frame(coordinates.sim1),kern.smooth = kern.smoothinZ, R = Rin)
    #z2.sim <- humboldt.grid.espace(z2$glob.g, z2$glob.s, data.frame(coordinates.sim2),kern.smooth = kern.smoothinZ, R = Rin)
    #o.i <- humboldt.niche.similarity(z1.sim, z2.sim, correct.env = cor.env.n, nae = nacinZ, thresh.espace.z = threshinZ)  # overlap between random and observed niches
	z1.sim <- humboldt::humboldt.grid.espace(z1_glob, z1_glob, data.frame(coordinates.sim1),kern.smooth = kern.smoothinZ, R = Rin)
    z2.sim <- humboldt::humboldt.grid.espace(z2_glob, z2_glob, data.frame(coordinates.sim2),kern.smooth = kern.smoothinZ, R = Rin)
    #observed niches
	o.i <- humboldt::humboldt.niche.similarity(z1=z1.sim,z2=z2.sim, correct.env = cor.envn, nae = nacinZ, thresh.espace.z = threshinZ)  # overlap between random and observed niches
    ...
    sim.o.D <- o.i$D[1]  # storage of overlaps
    sim.o.I <- o.i$I[1]
	#print(o.i)
...
humboldt.background.stat<- function(g2e, rep = 100, sim.dir = 1, env.reso, kern.smooth = 1, R = 100,
    ...
    z1 <- humboldt::humboldt.grid.espace(scores.env12a[,1:2], scores.env1a[,1:2], scores.sp1a[,1:2], kern.smooth = kern.smoothinZ, 
        R = Rin)
    z2 <- humboldt::humboldt.grid.espace(scores.env12a[,1:2], scores.env2a[,1:2], scores.sp2a[,1:2], kern.smooth = kern.smoothinZ, 
        R = Rin)
	l <- list()
	...
	o.i <- humboldt::humboldt.niche.similarity(z1, z2, correct.env = inE, nae = "YES", thresh.espace.z = threshinZ)
    #store obsereved
    obs.o$D <- o.i$D
    obs.o$I <- o.i$I
    switch(Sys.info()[['sysname']],
	...
	if (ncores == 1){
		if (force.equal.sample==T){
			...
			sim.o <- as.data.frame(matrix(unlist(lapply(1:rep, background.iter.e,scores.env12a, scores.env1a, scores.sp1a, scores.env2a, scores.sp2a, scores.env2c, Rana.x, Rana.y, env.reso.sim, run.silent.bak, inE, Rin, ncores, userOS,  threshinZ, kern.smoothinZ, pb, repT)), byrow = TRUE, ncol = 9))
		...
		if (force.equal.sample==F){
			if (run.silent.bak == F & userOS==1) {pb <- winProgressBar(title = "Initializing",min = 0,max =repT, width = 300)}
			if (run.silent.bak == F & userOS==2) {pb <-tkProgressBar(title = "Initializing", label = "", min = 0, max = repT, initial = 0, width = 300)}
			sim.o <- as.data.frame(matrix(unlist(lapply(1:rep, background.iter.r,scores.env12a, scores.env1a, scores.sp1a, scores.env2a, scores.sp2a, scores.env2c, Rana.x, Rana.y, env.reso.sim, run.silent.bak, inE, Rin, ncores, userOS,  threshinZ, kern.smoothinZ, pb, repT)), byrow = TRUE, ncol = 9))}  #simulate random overlap
			#number of cores attributed for the permutation test
			}
	if (ncores > 1) {
		future::plan(future::multisession, workers = ncores)
		...
		iter_fun <- if (force.equal.sample) background.iter.e else background.iter.r
		progressr::handlers("progress")
		progressr::handlers(global=TRUE)
		# Run simulations
		progressr::with_progress({
		  ...
		  sim_list <- future.apply::future_lapply(
		   seq_len(rep),
		   function(repi, ...) {
			# compute locally inside worker
			tryCatch({
				res <- iter_fun(repi, ...)
				p()
				res
			  }, error = function(e) {
				message("Worker failed at iteration ", repi, ": ", e$message)
				return(rep(NA, 9))
			  })
			},
			scores.env12a = scores.env12a,
			scores.env1a = scores.env1a,
			scores.sp1a = scores.sp1a,
			scores.env2a = scores.env2a,
			scores.sp2a = scores.sp2a,
			scores.env2c = scores.env2c,
			Rana.x = Rana.x,
			Rana.y = Rana.y,
			env.reso.sim = env.reso.sim,
			run.silent.bak = run.silent.bak,
			inE = inE,
			Rin = Rin,
			ncores = ncores,
			userOS = userOS,
			...
			pb = NULL,
			repT = repT,
			future.seed = TRUE# IMPORTANT for reproducibility
		)
...
background.iter.r<- function(repi, scores.env12a, scores.env1a, scores.sp1a, scores.env2a, scores.sp2a, scores.env2c, Rana.x, Rana.y, env.reso.sim, run.silent.bak, inE, Rin, ncores, userOS, threshinZ, kern.smoothinZ, pb, repT) {	
	...
	if (nsp2r > 4) {
        z1 <- humboldt::humboldt.grid.espace(scores.env12a[,1:2], scores.env1a[,1:2], scores.sp1a[,1:2], kern.smooth = kern.smoothinZ, R = Rin)
...
background.iter.e<- function(repi, scores.env12a, scores.env1a, scores.sp1a, scores.env2a, scores.sp2a, scores.env2c, Rana.x, Rana.y, env.reso.sim, run.silent.bak, inE, Rin, ncores, userOS, threshinZ, kern.smoothinZ, pb, repT) {
	sp2rPCAs2<-NULL
	...
	repeat { 
		...
		if (nsp2r > nsp2t || iterZ > max_iterZ) break
			iterZ <- iterZ + 1
        }  
	if (nsp2r >= nsp2t) {
...
humboldt.plot.niche <- function(z, title = "", name.axis1 ="PC1", name.axis2 = "PC2", correct.env = F, color.ramp=1) {
    ...
    if (correct.env == F) 
        contour(z$x, (sort((z$y ))), z$z.uncor, add = T, levels = quantile(z$z.uncor[z$z.uncor > 
            ...
            drawlabels = F, col = scales::alpha("white", 0.75))
    if (correct.env == T) 
...
humboldt.plot.contrib <- function(contrib = pca.cal$co,eigen = pca.cal$eig, pcx=1, pcy=2) {
...
humboldt.occ.rarefy <- function(in.pts, colxy = 2:3, rarefy.dist = 0, rarefy.units = "km", run.silent.rar = F) {
	...
	if (rarefy.units == "km") {
        ...
        sp2p1 <- sp::SpatialPoints(in.pts[, colxy], CRS("+proj=longlat +datum=WGS84"))
        sp2p2 <- sp::spTransform(sp2p1, CRS("+proj=aeqd +lat_0=0 +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"))
    ...
    repeat {
        nn1 <- spatstat.geom::nndist(del.min.dist[, "x4r"], del.min.dist[, "y4r"])  # calculate distance nearest neighbour
        ...
        # nearest neighbour
        nn2 <- spatstat.geom::nndist(del.min.dist[, "x4r"], del.min.dist[, "y4r"], k = 2)
        ...
        if (sum(del2) > 1) {
            for (k in 3:6) {
                nn <- spatstat.geom::nndist(del.min.dist[, "x4r"], del.min.dist[, "y4r"], k = k)
...
humboldt.plot.overlap <- function(in.g2e = zz, pdfname = "OverLapPlot.pdf", pdf.out = T, swap = F, pcx=1, pcy=2) {
    ...
    if (plot.axis == TRUE) 
        gridExtra::grid.arrange(plot_top, empty, grob1, plot_right, ncol = 2, nrow = 2, widths = c(4, 1), 
            heights = c(1, 4)) else grid.draw(grob1)
    if (pdf.out == TRUE) 
        ggsave(pdfname, gridExtra::grid.arrange(plot_top, empty, grob1, plot_right, ncol = 2, nrow = 2, 
...
#################################################################################################
### partically fixed 8/28/2025, test other options redux 1-3,5, also shapefile mask
###############################################################################################
##################################################################################################
##################################################################################################
...
humboldt.g2e <- function(env1, env2, sp1, sp2, reduce.env = 2, reductype = "PCA", non.analogous.environments = "NO", nae.window=5,
    ...
    if (env.trim== T & env.trim.type=="MCP") {
        print("Starting reduction of G-space using MCP method") 
		if ((nrow(env1)+nrow(env2))>20000){print(paste("You input", (nrow(env1)+nrow(env2)), "sites in your environment files, this could take a while"))}
		if ((nrow(env1)+nrow(env2))>100000){print(paste("Seriously, go grab a snack and drink"))}		
        options(warn = -1)
        CmaxF <- max(env1[, 2])
        CminF <- min(env1[, 2])
        DmaxF <- max(env2[, 2])
		...
		OcSp1T <- sp::SpatialPoints(sp1[, 2:3], proj4string = crs_wgs84)
		OcSp1Tmcp <- adehabitatHR::mcp(OcSp1T, percent = 100)
        OcSp1Tmcp <- sp::spTransform(OcSp1Tmcp, sp::CRS("+proj=longlat +datum=WGS84"))
		OcSp1Tmcp <- terra::vect(OcSp1Tmcp)
        OcSp1TmcpB <- terra::buffer(OcSp1Tmcp, width = trim.buffer.sp1v)
        env1Tmcp <- sp::SpatialPoints(env1in, sp::CRS("+proj=longlat +datum=WGS84"))
		env1Tmcp <- terra::vect(env1Tmcp)
		...
		# Sp2
        env2in <- env2
        OcSp2T <- sp::SpatialPoints(sp2[, 2:3], proj4string = crs_wgs84)
        OcSp2Tmcp <- adehabitatHR::mcp(OcSp2T, percent = 100)
        OcSp2Tmcp <- sp::spTransform(OcSp2Tmcp, sp::CRS("+proj=longlat +datum=WGS84"))
		OcSp2Tmcp <- terra::vect(OcSp2Tmcp)
        OcSp2TmcpB <- terra::buffer(OcSp2Tmcp, width = trim.buffer.sp2v)
        env2Tmcp <- sp::SpatialPoints(env2in, sp::CRS("+proj=longlat +datum=WGS84"))
        env2Tmcp <- terra::vect(env2Tmcp)
		...
		print("Finished reduction of G-space using MPC method")    
	...
	if (env.trim== T & env.trim.type=="RADIUS"){
        print("Starting reduction of G-space using Radius method")  
		if ((nrow(env1)+nrow(env2))>20000){print(paste("You input", (nrow(env1)+nrow(env2)), "sites in your environment files, this could take a while"))}
		if ((nrow(env1)+nrow(env2))>100000){print(paste("Seriously, go grab a snack and drink"))}
        options(warn = -1)
        # Sp1
		...
		crs_wgs84 <- sp::CRS("+proj=longlat +datum=WGS84 +no_defs")
		OcSp1T <- sp::SpatialPoints(sp1[, 2:3], proj4string = crs_wgs84)
		OcSp1T <- terra::vect(OcSp1T)
        OcSp1TmcpB <- terra::buffer(OcSp1T, width = trim.buffer.sp1v)
        env1Tmcp <- env1in[, 1:2]
		env1Tmcp <- sp::SpatialPoints(env1in[, 1:2], proj4string = crs_wgs84)
		env1Tmcp <- terra::vect(env1Tmcp)
        env1sdf <- env1Tmcp[OcSp1TmcpB, ]
        #env1 <- as.data.frame(env1sdf)
		env1_ptsC <- as.data.frame(env1sdf, geom = "XY")
        ...
        env2in <- env2
        OcSp2T <- sp::SpatialPoints(sp2[, 2:3],  proj4string = crs_wgs84)
		OcSp2T <- terra::vect(OcSp2T)
        OcSp2TmcpB <- terra::buffer(OcSp2T, width = trim.buffer.sp2v)
        env2Tmcp <- env2in[, 1:2]
		env2Tmcp <- sp::SpatialPoints(env2in,  proj4string = crs_wgs84)
		env2Tmcp <- terra::vect(env2Tmcp)
        env2sdf <- env2Tmcp[OcSp2TmcpB, ]
        #env2 <- as.data.frame(env2sdf)
        env2_ptsC<- as.data.frame(env2sdf, geom = "XY")
    ...
    ## approrpiate PCA
    pca.cal <- ade4::dudi.pca(data.env.occ, row.w = row.w.IN, center = T, scale = T, scannf = F, nf = nf.pcs)
    ...
    ##CURRENT WORKING STOP
	# flag same environments
    z3 <- humboldt.grid.espace(glob.g= scores.env12[1:2], glob.s = scores.env1[1:2], sp = scores.env1[1:2], 
        kern.smooth = kern.smoothinZ, R = R)
    z4 <- humboldt.grid.espace(glob.g= scores.env12[1:2], glob.s = scores.env2[1:2], sp = scores.env2[1:2], 
        kern.smooth = kern.smoothinZ, R = R)
    ...
    if (REDUC == 3) {
		...
		if( ratiozz_SP2 > 0.74){
			...
			print("In our past analyses, it has been commonly observed that the Niche Overlap Test (NOT, sensu Brown and Carnaval 2019) will be significant, but not NDT in these situations. However if you had a similar situation with the same large-range species and another species with a small or medium range – both the NDT and NOT test are significant.")    
			print("*****************************************************************")
			print("*****************************************************************")
			print("What to do:")   
			print("*****************************************************************")
			print("We recommend that when performing NDT, if reducing E-space to shared analogous environments reduces the number of occurrences to less than 75% of input points, the authors do not perform a NDT test.") 
			print("*****************************************************************")
			print("*****************************************************************")
			print("State this in methods:")
			print("*****************************************************************")
			print("Niche Divergence tests (Brown and Carnaval 2019) were not performed on species pair where shared analogous E-space reduced the input points of one of the species to less than 25% of input localities (after rarefying).  The Niche Divergence tests have been shown to be unreliable in these situations (cite Humboldt GitHub – this will be explained in a manuscript in the near future and this citation will be updated then).  In these situations, we present the results of Niche Overlap Tests (Brown and Carnaval 2019) as the final analyses of Niche Divergence.")   
			print("*****************************************************************")
			print("*****************************************************************")
			print("Do this in associated tables:")
			print("*****************************************************************")
			print("Denote any associated results with an asterix(e.g. D= 0.145*,p = 0.056*,... )")
			print("*****************************************************************")
			print("*****************************************************************")
			print("In table legends state:")
			print("*****************************************************************")
			print("*Due to a quantitative limitation of the NDT when comparing species with dramatically different distributions, particularly those with very small ranges (e.g., a large vs. a tiny range), NOT tests were not run in these species comparisons, instead we report the results of the NOT as final tests per software recommendation.")  
            print("*****************************************************************")
            print("*****************************************************************")
			}
...
#' @param select.k If equals TRUE then this will plot Espace at several kernel smoothing values and allow you to select the best value for downstream analyses.  If FALSE, this will be skipped and input kern.smooth value will be used for analyses.
...
#' @seealso \code{humboldt.g2e, humboldt.equivalence.stat, humboldt.background.stat, humboldt.niche.similarity, humboldt.plot.niche, humboldt.doitall, humboldt.top.env.brt} which use or depend on outputs of this function 
#' @import terra
#' @export
#' @examples
#' #######################################################################################
...
humboldt.doitall <- function(inname = "DoItAll", env1, env2, sp1, sp2, rarefy.dist = 0, rarefy.units = "dd", env.reso, 
    ...
    humboldt.plot.niche(z1, correct.env = F, title = "E-space Environment 1", color.ramp= color.ramp, name.axis1 = paste("PC",pcx,"k=",kern.smoothinZ,sep = ""), 
        name.axis2 = paste("PC",pcy, sep = ""))
    humboldt.plot.niche(z2, correct.env = F, title = "E-space Environment 2", color.ramp= color.ramp, name.axis1 = paste("PC",pcx,"k=",kern.smoothinZ,sep = ""), 
        name.axis2 = paste("PC",pcy, sep = ""))
    ## measure differences
    ...
    #    ymx = max(df[, 2]))
    cols <- colr.o
    cells <- terra::cellFromXY(r3, df[, 1:2])
    r3[cells] <- df[, 3]
    ...
    #    ymx = max(df[, 2]))
    cols <- colr.o
    cells <- terra::cellFromXY(r4, df[, 1:2])
    r4[cells] <- df[, 3]
    ...
    #    ymx = max(df[, 2]))
    cols <- colr.o
    cells <- terra::cellFromXY(r1, df[, 1:2])
    r1[cells] <- df[, 4]
    ...
    #    ymx = max(df[, 2]))
    cols <- colr.o
    cells <- terra::cellFromXY(r2, df[, 1:2])
    r2[cells] <- df[, 4]
	...
	print("Species 1:")
	pnt1<-pnt2<-NULL
	pnt1$pnt.index<-0;pnt2$pnt.index<-0
...
##################################################################################################
#######################Select top environmental variables for PCA - boosted regression trees
##################################################################################################
...
humboldt.top.env.brt <- function(env1, env2, sp1, sp2, rarefy.dist = 0, rarefy.units="km", env.reso, learning.rt1 = 0.01, 
    ...
    contrib.greater = 5) # method=='estimate' method=='contrib' method=='nvars'
{
    ...
    if (method == "nvars" || method == "contrib") {
        modelOutA <- humboldt.gbm.step(data = datamodel, gbm.x = x2var, gbm.y = 1, family = "bernoulli", 
    ...
    if (method == "nvars" || method == "contrib") {
        modelOutB <- humboldt.gbm.step(data = datamodel, gbm.x = x2var, gbm.y = 1, family = "bernoulli", 
...
##################################################################################################
#' Visualize and select kernel smoothing parameter
#' @param input.g2e This is the output from humboldt.g2e 
#' @param k.min The minimum k-value to evalute (default=0.25)
#' @param k.max The maximum k-value to evalute (default=2)
#' @param k.step The step value to evaluated k-values between the mmaximum and minimum (default=0.25, this means with other defaults, this function will look at the follwing k-values: 0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2)
#' @return This tool returns selected K-value for further use. 
#' @export
#' @examples
#' library(humboldt)
#'
...
#' 
#' ##perform modeling to determin imporant variables
#' reduc.vars<- humboldt.top.env.maxent(env1=env1,env2=env2,sp1=occ.sp1,sp2=occ.sp2,rarefy.dist=40, rarefy.units="km",env.reso=0.0833338,e.var=(3:21),pa.nu=10000,method="contrib",contrib.greater=5)
#' 
#' ##use new variables for env1 and evn2, use as you normally would do for env1/env2 (input above)
#' 
#' ##for example, input into converted geographic space to espace
#' zz<-humboldt.g2e(env1=reduc.vars$env1, env2=reduc.vars$env2...)
#' ## select best kernel smoothing parameter based on plots
#' kern.smooth<-humboldt.smoothing.eval <- function(input.g2e, k.min=0.25,k.max=2,k.step=0.25)

humboldt.smoothing.eval <- function(input.g2e, k.min=0.25,k.max=2,k.step=0.25) {
scores.env1<-input.g2e$scores.env1[1:2]
scores.env2<-input.g2e$scores.env2[1:2]
scores.env12<- rbind(input.g2e$scores.env1[1:2],input.g2e$scores.env2[1:2])
scores.sp1<-input.g2e$scores.sp1[1:2]
scores.sp2<-input.g2e$scores.sp2[1:2]

#get list of k parameters
k_eval <- seq(k.min, k.max, by = k.step)
val_k <- numeric(length(k_eval))


for (i in 1:length(val_k)){
	## run create a grid of Environmental Space Function
	z1<- humboldt.grid.espace(scores.env12,scores.env1,scores.sp1,kern.smooth=k_eval[i],R=100)
	z2<- humboldt.grid.espace(scores.env12,scores.env2,scores.sp2,kern.smooth=k_eval[i],R=100)
	#'
	## plot niche in espace
	par(mfrow= c(1,2))
	humboldt.plot.niche(z1,paste("Species 1: Smoothing K=",k_eval[i]),"PC1","PC2")
	humboldt.plot.niche(z2,paste("Species 2: Smoothing K=",k_eval[i]),"PC1","PC2")
...
print("*************************************************")
print("*************************************************")
kern.smooth.future<-NULL

message(strrep("*", 50))
  message("Review plots to choose appropriate smoothing (K).")
  message(strrep("*", 50))

  # Robust input loop
  repeat {
    input <- readline(
      prompt = "Enter best kernel smoothing value: "
    )
    k_future <- suppressWarnings(as.numeric(input))

    if (!is.na(k_future)) break
...
#' @param method this determines how important environmental variables are selected.  There are two options: "contrib", "nvars". If method="contrib", variables above a relative influence value will be kept. See associated parameter 'contrib.greater'. If method="nvars", a fixed number of user specified variables will be kept. The kept variables are selected by their relative influence, selecting for the highest contributing variables. See associated parameter 'nvars.save'
#' @param contrib.greater if method="contrib", this variable is required. The kept variables are selected for their relative influence in predicting the species' distribution.  Here users select variables equal to or above an input model contribution value. The default value for this method is 5 (= variables with 5 percent or higher contribution to model of either species are kept). This value will be ignored if method="nvars"
#' @param pa.nu number of background points, typically this is 10,000. The null value is 10,000.
#' @return This function runs generalized boosted regression models (a machine learning SDM algorithm) to select top parameters for inclusion in PCA. This is important because you want the PC to reflect variables that are relevant to the species distribution. Alternatively you can run Maxent outside of R and manually curate the variables you include (also recommended). 
#' @import dplyr
#' @import sp
#' @import enmSdmX
...
humboldt.top.env.maxent <- function(env1, env2, sp1, sp2, rarefy.dist = 0, rarefy.units="km", env.reso, e.var, pa.nu = 10000, method = "contrib", nvars.save = 5, contrib.greater = 5) # method=='contrib' method=='nvars'
{
    ...
    sp <- cbind(ID, occ.sp1)
    datamodel <- rbind(sp, clim)
    print("Running MaxEnt model for species 1")
    if (method == "nvars" || method == "contrib") {
		maxpreds<-ncol(datamodel)-1
		modelOutA <- enmSdmX::trainMaxEnt(
		data = datamodel,
		resp = names(datamodel)[1],
		preds = names(datamodel)[4:maxpreds],
		regMult = 1, # too few values for reliable model, but fast
		verbose = TRUE,
		cores = 1
		)}
    if (method == "nvars") {
        a<-as.data.frame(modelOutA@results)
		aa<-grep(pattern="*contribution",x = rownames(a), value = TRUE)
		aaa<- a %>% filter(row.names(a) %in% c(aa))
		aaaa<-dplyr::arrange(aaa,desc(aaa$V1))
		aaaaa<-rownames(aaaa)[1:nvars.save]
		varInc1<<-gsub(pattern="*.contribution", replacement ="", x = aaaaa)
		}
    if (method == "contrib") {
        a<-as.data.frame(modelOutA@results)
		aa<-grep(pattern="*contribution",x = rownames(a), value = TRUE)
		aaa<- a %>% filter(row.names(a) %in% c(aa))
		aaaa<-dplyr::arrange(aaa,desc(aaa$V1))
		aaaaa<-rownames(aaaa)[aaaa>contrib.greater]
		varInc1<<-gsub(pattern="*.contribution", replacement ="", x = aaaaa)
		}
    ## sp2
    climSample <- env2[sample(nrow(env2), nnpa2), ]
    ...
    sp <- cbind(ID, occ.sp2)
    datamodel <- rbind(sp, clim)
    print("Running MaxEnt model for species 2")
    if (method == "nvars" || method == "contrib") {
		maxpreds<-ncol(datamodel)-1
		modelOutB <- enmSdmX::trainMaxEnt(
		data = datamodel,
		resp = names(datamodel)[1],
		preds = names(datamodel)[4:maxpreds],
		regMult = 1, # too few values for reliable model, but fast
	...
	if (method == "nvars") {
        a<-as.data.frame(modelOutB@results)
		aa<-grep(pattern="*contribution",x = rownames(a), value = TRUE)
		aaa<- a %>% filter(row.names(a) %in% c(aa))
		aaaa<-dplyr::arrange(aaa,desc(aaa$V1))
		aaaaa<-rownames(aaaa)[1:nvars.save]
		varInc2<<-gsub(pattern="*.contribution", replacement ="", x = aaaaa)
		}
    if (method == "contrib") {
        a<-as.data.frame(modelOutB@results)
		aa<-grep(pattern="*contribution",x = rownames(a), value = TRUE)
    ...
    # begin partition environment
    env1sp1 <- subset(env1, select = c(varInc1))
    env1sp2 <- subset(env1, select = c(varInc2))
    env2sp1 <- subset(env2, select = c(varInc1))
    env2sp2 <- subset(env2, select = c(varInc2))
    # reconstruct original environmental variables with xy
    env1pre <- cbind(env1[1:2], env1sp1, env1sp2)
    env2pre <- cbind(env2[1:2], env2sp1, env2sp2)
    # store new environment columns and remove duplicate names
...
#  assign("y.data", y.data, env = globalenv())
	...
	if (!silent) {
		cat("\n","\n","GBM STEP - version 2.9: J. Leathwick & J. Elith","\n","\n")
		cat("****FUNCTION FROM RETIRED DISMO PACKAGE**** \n")
...
# calculate the total deviance
	...
	total.deviance <- humboldt.calc.deviance(y_i, u_i, weights = site.weights, family = family, calc.mean = FALSE)
...
# now step through the folds setting up the initial call
	...
	# calc training deviance
		...
		training.loss.matrix[i,1] <- humboldt.calc.deviance(y_i, u_i, weight.fitted, family = family)
...
# calc holdout deviance
		...
		cv.loss.matrix[i,1] <- humboldt.calc.deviance(y_i, u_i, weight.preds, family = family)
...
# calculate training deviance
			...
			u_i <- fitted.values
			weight.fitted <- site.weights[model.mask]
			training.loss.matrix[i,j] <- humboldt.calc.deviance(y_i, u_i, weight.fitted, family = family)
...
# calc holdout deviance
			...
			y_i <- y.data[pred.mask]
			weight.preds <- site.weights[pred.mask]
			cv.loss.matrix[i,j] <- humboldt.calc.deviance(y_i, u_i, weight.preds, family = family)
...
# includes estimates of the standard error of the fitted values added 2nd may 2005
	...
	for (i in 1:n.folds) {
		...
		cv.deviance.stats[i] <- humboldt.calc.deviance(y_i, u_i, weight.preds, family = family)

		cv.cor.stats[i] <- cor(y_i,u_i)

		if (family == "bernoulli") {
			cv.roc.stats[i] <- humboldt.roc(y_i,u_i)
			cv.calibration.stats[i,] <- humboldt.calibration(y_i,u_i,"binomial")
			threshold.stats[i] <- approx(ppoints(u_i), sort(u_i,decreasing = T), prevalence)$y
		}

		if (family == "poisson") {
			cv.calibration.stats[i,] <- humboldt.calibration(y_i,u_i,"poisson")
		}
	}
...
#extract fitted values and summary table
	...
	y_i <- y.data
	u_i <- fitted.values
	resid.deviance <- humboldt.calc.deviance(y_i, u_i, weights = site.weights, family = family, calc.mean = FALSE)
	...
	if (family == "bernoulli") {  # do this manually as we need the residuals
		...
		self.roc <- humboldt.roc(y_i,u_i)
		self.calibration <- humboldt.calibration(y_i,u_i,"binomial")
	}
	...
	if (family == "poisson") {   # do this manually as we need the residuals
		...
		residuals <- sqrt(abs(deviance.contribs * 2))
		residuals <- ifelse((y_i - u_i) < 0, 0 - residuals, residuals)
		self.calibration <- humboldt.calibration(y_i,u_i,"poisson")
	}
...
humboldt.calc.deviance <-  function(obs, pred, weights = rep(1,length(obs)), family="binomial", calc.mean = TRUE) {
...
humboldt.roc <-function (obsdat, preddat) {
...
humboldt.calibration <- function(obs, preds, family = "binomial")  {
#
