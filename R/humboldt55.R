...
humboldt.equivalency.test <- function(z1, z2, rep = 100, correct.env = T, kern.smooth = 1, nae = "YES", 
    ...
    if (sample == "density") {
        if (correct.env == T) {
            obs.o <- humboldt.niche.similarity(z1, z2, correct.env = T, nae = nacinZ, thresh.espace.z = threshinZ)  #observed niche similarity
            sim.o <- data.frame(matrix(nrow = rep, ncol = 2))  #empty list of random niche similarity
            names(sim.o) <- c("D", "I")
            if (!is.null(z1$y)) {
                # overlap on two axes
                ...
                o.i <- humboldt.niche.similarity(z1.sim, z2.sim, correct.env = T, nae = nacinZ, 
        ...
        if (correct.env == F) {
            obs.o <- humboldt.niche.similarity(z1, z2, correct.env = F, nae = nacinZ, thresh.espace.z = threshinZ)  #observed niche similarity
            sim.o <- data.frame(matrix(nrow = rep, ncol = 2))  #empty list of random niche similarity
            names(sim.o) <- c("D", "I")
            for (i in 1:rep) {
                if (run.silent.equ == F) {
                ...
                o.i <- humboldt.niche.similarity(z1.sim, z2.sim, correct.env = F, nae = nacinZ, 
...
humboldt.background.test<- function(g2e, rep = 100, sim.dir = 1, env.reso, kern.smooth = 1, 
    ...
    l <- list()
    sim.o <- data.frame(matrix(nrow = rep, ncol = 9))  #empty list of random niche similarity
    ## jlb added
    ...
    o.i <- humboldt.niche.similarity(z1, z2, correct.env = inE, nae = "YES")
    if (force.equal.sample==T){
      for (k in 1:rep) {
        ...
        o.ot <- humboldt.niche.similarity(z1, z2.ran, correct.env = inE, nae = "YES")
...
#' full<-humboldt.doitall(inname="full_extent", env1=reduc.vars$env1, env2=reduc.vars$env2, sp1=sp1, sp2=sp2, rarefy.dist=50, rarefy.units="km", env.reso=0.416669, reduce.env=0, reductype="PCA", non.analogous.environments="YES", correct.env=T, env.trim=F,  env.trim.type="RADIUS", trim.buffer.sp1=200, trim.buffer.sp2=200, pcx=1, pcy=2, col.env=e.var, e.var=c(3:num.var.e), R=100, kern.smooth=1, PROJ=F, e.reps=100, e.sample.method="points", b.reps=100, env.pca=T, nae="YES",thresh.espace.z=0.001, p.overlap=T, p.boxplot=F, p.scatter=F, run.silent=F)
#'
...
#' shared_ae<-humboldt.doitall(inname="shared_espace_ae", env1=reduc.vars$env1, env2=reduc.vars$env2, sp1=sp1, sp2=sp2, rarefy.dist=50, rarefy.units="km", env.reso=0.416669, reduce.env=2, reductype="PCA", non.analogous.environments="NO", correct.env=T, env.trim=T, env.trim.type="RADIUS", trim.buffer.sp1=200, trim.buffer.sp2=200, pcx=1,pcy=2, col.env=e.var, e.var=c(3:num.var.e), R=100, kern.smooth=1, PROJ=F, e.reps=100, e.sample.method="points", b.reps=100, env.pca=T, nae="YES",thresh.espace.z=0.001, p.overlap=T, p.boxplot=F, p.scatter=T,run.silent=F)
