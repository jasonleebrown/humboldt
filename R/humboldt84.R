##tcltk, snow, parallel, pbapply, terra, ade4,sp,
...
humboldt.equivalence.stat <- function(z1, z2, rep = 100, correct.env = T, kern.smooth = 1, nae = "YES", 
	...
	l <- list()
	obs.o <- humboldt.niche.similarity(z1, z2, correct.env= cor.envn, nae = nacinZ, thresh.espace.z = threshinZ)  #observed niche similarity
	if (ncores == 1){
		...
		sim.o <- as.data.frame(matrix(unlist(lapply(1:rep, equivalency.iter, z1, z2, nacinZ, threshinZ, kern.smoothinZ, Rin,pb, repT, ncores, run.silent.equ, cor.envn,userOS)), byrow = TRUE, ncol = 2))}  #simulate random overlap
		#number of cores attributed for the permutation test
	if (ncores > 1){
	    cl <- snow::makeCluster(ncores)  #open a cluster for parallelization
		snow::clusterExport(cl=cl, c("z1", "z2"), envir=environment())
		invisible(snow::clusterEvalQ(cl, library(humboldt)))
		...
		sim.o <- as.data.frame(matrix(unlist(pbapply::pblapply(1:rep, equivalency.iter, z1, z2, nacinZ, threshinZ, kern.smoothinZ, Rin, pb, repT, ncores,run.silent.equ, cor.envn, userOS, cl=cl)), byrow = TRUE, ncol = 2)) #simulate random overlap
		snow::stopCluster(cl)  #shutdown the cluster
	}		
...
equivalency.iter<- function(repi, z1, z2, nacinZ, threshinZ, kern.smoothinZ, Rin, pb, repT, ncores, run.silent.equ, cor.envn, userOS) {
	...
	z1.sim <- humboldt::humboldt.grid.espace(z1$glob.g, z1$glob.g, data.frame(coordinates.sim1),kern.smooth = kern.smoothinZ, R = Rin)
    z2.sim <- humboldt::humboldt.grid.espace(z2$glob.g, z2$glob.g, data.frame(coordinates.sim2),kern.smooth = kern.smoothinZ, R = Rin)
    #observed niches
	o.i <- humboldt.niche.similarity(z1=z1.sim,z2=z2.sim, correct.env = cor.envn, nae = nacinZ, thresh.espace.z = threshinZ)  # overlap between random and observed niches
