...
humboldt.pnt.index <- function(glob.g, glob.s, sp, R = 100, kern.smooth = 1) {
    ...
    if (ncol(glob.g) == 2) {
        ...
        for (i in 1:length(Benv)) {		
           ...
           xyB<-cbind(Bx,By)
           p = sp::Polygon(xyB)
           ps1 = sp::Polygons(list(p),1)
           if (i ==1) {sBenv = sp::SpatialPolygons(list(ps1))}
           if (i !=1) {sBenv = sp::SpatialPolygons(list(ps1))+sBenv}
           }
        ...
        for (i in 1:length(Bsp)) {
           ...
           p = sp::Polygon(xyA)
           ps1 = sp::Polygons(list(p),1)
           if (i ==1) {sBsp = sp::SpatialPolygons(list(ps1))}
           if (i !=1) {sBsp = sp::SpatialPolygons(list(ps1))+sBsp}
           }
        ...
        ratio<-area.red<- try(((0.5*sum(geosphere::lengthLine(sBsD)))/sum(geosphere::lengthLine(sBsp))), silent = TRUE)
			if (inherits(ratio, 'try-error')) {ratio<-0}
        #borders = gDifference(
        #   as(sBf,"SpatialLines"),
        #   as(gUnaryUnion(sBf),"SpatialLines"),
        #   byid=TRUE)
        #bordersL<-geosphere::lengthLine(borders)
        #bordersF<-geosphere::lengthLine(sBsp)
...
#'
#' ##its highly recommened that you using the function "humboldt.top.env" to select only the important enviromnetal variables in humboldt.doitall. This step can be skipped. If you downloaded tons of environmental data, you should use this step.  If you skip this step, input env1/env2 inplace of reduc.vars$env1/reduc.vars$env2 
#' reduc.vars<- reduc.vars<- humboldt.top.env(env1=env1,env2=env2,sp1=sp1,sp2=sp2,rarefy.dist=40, rarefy.units="km",env.reso=0.0833338,e.var=(3:21),pa.nu=10000,method="contrib",contrib.greater=5)
#' 
#' ##Adjust the number of variables input for e.vars after reduction to only important variables
#' num.var.e<-ncol(reduc.vars$env1)
...
humboldt.top.env <- function(env1, env2, sp1, sp2, rarefy.dist = 0, rarefy.units="km", env.reso, e.var, pa.nu = 10000, method = "contrib", nvars.save = 5, contrib.greater = 5) # method=='contrib' method=='nvars'
{
