...
humboldt.equivalency.test <- function(z1, z2, rep = 100, correct.env = T, kern.smooth = 1, nae = "YES", 
    ...
    if (sample == "density") {
        if (correct.env == T) {
            obs.o <- humboldt.niche.overlap(z1, z2, correct.env = T, nae = nacinZ, thresh.espace.z = threshinZ)  #observed niche similarity
        ...
        if (correct.env == F) {
            obs.o <- humboldt.niche.overlap(z1, z2, correct.env = F, nae = nacinZ, thresh.espace.z = threshinZ)  #observed niche similarity
            sim.o <- data.frame(matrix(nrow = rep, ncol = 2))  #empty list of random niche similarity
            names(sim.o) <- c("D", "I")
            for (i in 1:rep) {
                if (run.silent.equ == F) {
