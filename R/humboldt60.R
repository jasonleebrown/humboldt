...
#' @seealso \code{humboldt.equivalency.test, humboldt.background.test, humboldt.niche.similarity, humboldt.plot.niche,humboldt.doitall, humboldt.top.env} that use or depend on outputs of this function 
#' @import sp
#' @import raster
...
#' zz<-humboldt.g2e(env1=env1, env2=env2, sp1=occ.sp1, sp2=occ.sp2, reduce.env = 2, reductype = "PCA", non.analogous.environments = "NO", e.var=c(3:21),  col.env = e.var, env.trim= T, env.trim.type="MCP", trim.buffer.sp1 = 200, trim.buffer.sp2 = 200, env.pca = T, rarefy.dist = 50, rarefy.units="km", env.reso=0.41666669, kern.smooth = 1, PROJ = F,  R = 100, run.silent = F)
#' 
...
#' zz<-humboldt.g2e(env1=reduc.vars$env1, env2=reduc.vars$env2, sp1=occ.sp1, sp2=occ.sp2, reduce.env = 0, reductype = "PCA", non.analogous.environments = "YES", env.trim= T, e.var=c(3:num.var.e),  col.env = e.var, trim.buffer.sp1 = 200, trim.buffer.sp2 = 200, env.pca = T, rarefy.dist = 50, rarefy.units="km", env.reso=0.41666669, kern.smooth = 1, PROJ = F,  R = 100, run.silent = F)
#' 
#' ##store espace scores for sp1 and environments 1,2 and both environments combined output from humboldt.g2e
#' scores.env1<-zz$scores.env1[1:2]
#' scores.env2<-zz$scores.env2[1:2]
...
equivalency.iter<- function(repi, z1, z2, nacinZ, threshinZ, kern.smoothinZ, Rin, pb, repT, ncores, run.silent.equ, cor.env.n, userOS) {
    ...
    z1.sim <- humboldt::humboldt.grid.espace(z1$glob, z1$glob1, data.frame(coordinates.sim1),kern.smooth = kern.smoothinZ, R = Rin)
    z2.sim <- humboldt::humboldt.grid.espace(z2$glob, z2$glob1, data.frame(coordinates.sim2),kern.smooth = kern.smoothinZ, R = Rin)
...
#' @param ncores number of CPUs to use for tests. If unsure on the number of cores and want to use all but 1 CPU, input ncores="All"
#' @return Runs a modified niche background test(see Warren et al 2008) based on two species' occurrence density grids. The function compares the observed niche similarity between z1 and z2 (created by humboldt.grid.espace) to overlap between z1 and the random shifting of the spatial distribution of z2 in geographic space and then measuring how that shift in geography changes occupied environmental space (called z2.sim). This test maintains the spatial structure of all the localities and thereby retains all nuances associated with each datasets' spatial autocorrelation (vs. random sampling). This test asks if the two distributed populations/species are more different than would be expected given the underlying environmental differences between the regions in which they occur. \cr
#' \cr
...
#' @seealso \code{humboldt.g2e, humboldt.equivalency.test, humboldt.background.test, humboldt.niche.similarity, humboldt.plot.niche,humboldt.doitall} which use or depend on outputs of this function 
#' @import snow
#' @import pbapply 
#' @importFrom tcltk setTkProgressBar
...
humboldt.background.test<- function(g2e, rep = 100, sim.dir = 1, env.reso, kern.smooth = 1, R = 100,
    correct.env = F, thresh.espace.z = 0.0001, force.equal.sample=F, run.silent.bak = F, ncores=1) {
...
background.iter.e.old<- function(repi, z1, scores.env12a, scores.env2a, scores.sp2a, scores.env2c, Rana.x, Rana.y, env.reso.sim, run.silent.bak, inE, Rin, ncores, userOS, threshinZ, kern.smoothinZ, pb, repT) {
    ...
    z2.ran <- humboldt::humboldt.grid.espace(scores.env12a[1:2], scores.env2a[1:2], sp2rPCA, kern.smooth = kern.smoothinZ, R = Rin)
...
#' @seealso \code{humboldt.sample.spp,humboldt.g2e, humboldt.equivalency.test, humboldt.background.test, humboldt.niche.similarity, humboldt.plot.niche,humboldt.doitall} which use or depend on outputs of this function 
...
humboldt.plot.scatter <- function(x = NA, color.ramp = 1, dcol="black", lhist = 20, num.dnorm = 5 * lhist, ...) {
    ## check input
    stopifnot(ncol(x) == 2)
...
#' full=humboldt.g2e(env1=env1, env2=env2, sp1=occ.sp1, sp2=occ.sp2, reduce.env = 2, reductype = "PCA", non.analogous.environments = "NO", env.trim= T, env.trim.type= "MCP", e.var=c(3:21),  col.env = e.var, trim.buffer.sp1 = 200, trim.buffer.sp2 = 200, env.pca = T, rarefy.dist = 50, rarefy.units="km", env.reso=0.41666669, kern.smooth = 1, PROJ = F,  R = 100, run.silent = F)
#' 
...
#' occ.sp2<-na.exclude(read.delim("sp2.txt",h=T,sep="\t"))
#' 
#' zz=humboldt.g2e(env1=env1, env2=env2, sp1=occ.sp1, sp2=occ.sp2, reduce.env = 2, reductype = "PCA", non.analogous.environments = "NO", env.trim= T, env.trim.type= "MCP", e.var=c(3:21),  col.env = e.var, trim.buffer.sp1 = 200, trim.buffer.sp2 = 200, env.pca = T, rarefy.dist = 50, rarefy.units="km", env.reso=0.41666669, kern.smooth = 1, PROJ = F,  R = 100, run.silent = F)

humboldt.g2e <- function(env1, env2, sp1, sp2, reduce.env = 2, reductype = "PCA", non.analogous.environments = "NO", nae.window=5,
    env.trim = T, e.var, col.env = e.var, env.trim.type="MCP", trim.mask1, trim.mask2, trim.buffer.sp1 = 200, trim.buffer.sp2 = 200, env.pca = T, pcx=1, pcy=2, rarefy.dist  = 0, rarefy.units = "km" , env.reso, kern.smooth = 1, PROJ = F, R = 100, run.silent = F) {
    if (reduce.env == 0) {
        REDUC = 0
    }
...
#' @seealso \code{humboldt.g2e, humboldt.equivalency.test, humboldt.background.test, humboldt.niche.similarity, humboldt.plot.niche, humboldt.doitall, humboldt.top.env} which use or depend on outputs of this function 
#' @import raster
#' @export
#' @examples
#' #######################################################################################
...
#' full<-humboldt.doitall(inname="full_extent", env1=reduc.vars$env1, env2=reduc.vars$env2, sp1=sp1, sp2=sp2, rarefy.dist=50, rarefy.units="km", env.reso=0.416669, reduce.env=0, reductype="PCA", non.analogous.environments="YES", correct.env=T, env.trim=F,  env.trim.type="RADIUS", trim.buffer.sp1=200, trim.buffer.sp2=200, pcx=1, pcy=2, col.env=e.var, e.var=c(3:num.var.e), R=100, kern.smooth=1, PROJ=F, e.reps=100, b.reps=100, env.pca=T, nae="YES",thresh.espace.z=0.001, p.overlap=T, p.boxplot=F, p.scatter=F, run.silent=F, ncores=1, color.ramp=3)
#'
#' ##run it a second time with a trimmed, shared-espace. Here the equivalency test tests for niche evolution or niche divergence. For comparing results, change only the following model parameters: reduce.env, non.analogous.environmental, env.trim
#' shared_ae<-humboldt.doitall(inname="shared_espace_ae", env1=reduc.vars$env1, env2=reduc.vars$env2, sp1=sp1, sp2=sp2, rarefy.dist=50, rarefy.units="km", env.reso=0.416669, reduce.env=2, reductype="PCA", non.analogous.environments="NO", correct.env=T, env.trim=T, env.trim.type="RADIUS", trim.buffer.sp1=200, trim.buffer.sp2=200, pcx=1,pcy=2, col.env=e.var, e.var=c(3:num.var.e), R=100, kern.smooth=1, PROJ=F, e.reps=100, b.reps=100, env.pca=T, nae="YES",thresh.espace.z=0.001, p.overlap=T, p.boxplot=F, p.scatter=T,run.silent=F, ncores=1, color.ramp=3)
...
#' ##run it first with full environmental for backgroud tests and equivalency test (total equivalence or divergence in current distributions)
#' full<-humboldt.doitall(inname="full_extent", env1=reduc.vars$env1, env2=reduc.vars$env2, sp1=sp1, sp2=sp2, rarefy.dist=50, rarefy.units="km", env.reso=0.416669, reduce.env=0, reductype="PCA", non.analogous.environments="YES", correct.env=T, env.trim=F,  env.trim.type="RADIUS", trim.buffer.sp1=200, trim.buffer.sp2=200, pcx=1, pcy=2, col.env=e.var, e.var=c(3:num.var.e), R=100, kern.smooth=1, PROJ=F, e.reps=100, b.reps=100, env.pca=T, nae="YES",thresh.espace.z=0.001, p.overlap=T, p.boxplot=F, p.scatter=F, run.silent=F, ncores=1)
#'
#' ##run it a second time with a trimmed, shared-espace. Here the equivalency test tests for niche evolution or niche divergence. For comparing results, change only the following model parameters: reduce.env, non.analogous.environmental, env.trim
#' shared_ae<-humboldt.doitall(inname="shared_espace_ae", env1=reduc.vars$env1, env2=reduc.vars$env2, sp1=sp1, sp2=sp2, rarefy.dist=50, rarefy.units="km", env.reso=0.416669, reduce.env=2, reductype="PCA", non.analogous.environments="NO", correct.env=T, env.trim=T, env.trim.type="RADIUS", trim.buffer.sp1=200, trim.buffer.sp2=200, pcx=1,pcy=2, col.env=e.var, e.var=c(3:num.var.e), R=100, kern.smooth=1, PROJ=F, e.reps=100, b.reps=100, env.pca=T, nae="YES",thresh.espace.z=0.001, p.overlap=T, p.boxplot=F, p.scatter=T,run.silent=F, ncores=1)


humboldt.doitall <- function(inname = "DoItAll", env1, env2, sp1, sp2, rarefy.dist = 0, rarefy.units = "dd", env.reso, 
    reduce.env = 2, reductype = "PCA", non.analogous.environments = "NO", nae.window=5, env.trim= T, env.trim.type="MCP", trim.mask1, trim.mask2, trim.buffer.sp1 = 200, color.ramp=1,
    trim.buffer.sp2 = 200, correct.env = T, pcx=1, pcy=2, col.env = e.var, e.var, R = 100, kern.smooth = 1, 
    PROJ = F, e.reps = 100, b.reps = 100, b.force.equal.sample=F, env.pca = T, nae = "YES", 
    thresh.espace.z = 0.001, p.overlap = T, p.boxplot = F, p.scatter = F, run.silent = F, ncores=1) {
    ...
    zz <<- humboldt.g2e(env1, env2, sp1, sp2, reduce.env, reductype, non.analogous.environments, nae.window,
        env.trim, e.var, col.env = e.var,env.trim.type, trim.mask1, trim.mask2, trim.buffer.sp1, trim.buffer.sp2, env.pca, pcx=pcx, pcy=pcy, rarefy.dist,rarefy.units, env.reso, kern.smooth, PROJ, R, run.silent = run.silent)
    ##################################################################################################
    ...
    if (reduce.env != 0 & ptlss1 > 0.5 & nrow(zz$scores.sp2) < 10) {
        print(paste("Only ", nrow(zz$scores.sp2), " points remain in species dataset 2. In this analysis, it is recommended to not trim shared e-space or remove non-analogous climates because input habitats are too divergent.  Basically input environments are non-equivalent and have little e-space overlap relevant to species 2. Because e-space cannot be reduced to shared/non-analogous environments: report shared analogos space percentage, niche similarity values and equivalency test significance for full climate space (see below for correct.env parameter recommendations)"))
    }
    if (reduce.env != 0 & ptlss2 > 0.5 & nrow(zz$scores.sp1) < 10) {
        print(paste("Only ", nrow(zfz$scores.sp1), " points remain in species dataset 1. In this analysis, it is recommended to not trim shared e-space or remove non-analogous climates because input habitats are too divergent.  Basically input environments are non-equivalent and have little e-space overlap relevant to species 1. Because e-space cannot be reduced to shared/non-analogous environments: report shared analogos space percentage, niche similarity values and equivalency test significance for full climate space (see below for correct.env parameter recommendations)"))
    }
...
#' @seealso \code{humboldt.g2e, humboldt.grid.espace, humboldt.equivalency.test, humboldt.background.test, humboldt.niche.similarity, humboldt.plot.niche,humboldt.doitall} which use or depend on outputs of this function 
#' @export
#' @examples
#' library(humboldt)
#'
...
o.sim.b2<-0
l <- list()
print("Performing Equivalence and Background tests in accessible environments characterized over the following distances:")
print(round(ahe,0))
for (i in ahe.len){
	ahe.i<-ahe[i]
	zz<-humboldt.g2e(env1=env1, env2=env2, sp1=occ.sp1in, sp2=occ.sp2in, reduce.env = 2, reductype = "PCA", non.analogous.environments = "NO", env.trim= T, e.var=e.var, col.env = e.var, trim.buffer.sp1 = ahe.i, trim.buffer.sp2 = ahe.i, env.pca = T, rarefy.dist = 0, env.trim.type=env.trim.type,rarefy.units=rarefy.units, env.reso=env.reso, kern.smooth = kern.smoothinZ, PROJ = F,  R = Rin, run.silent = run.silent)
	...
	print(paste("Starting equivalency tests for distance:",round(ahe.i,0)))
	output.e<-humboldt.equivalency.test(z1, z2, rep, correct.env = T, kern.smooth = kern.smoothinZ, nae = "YES", 
	...
	print(paste("Starting background tests 1->2 for distance:",round(ahe.i,0)))
	output.b1<-humboldt.background.test(zz, rep, sim.dir = 1, env.reso=env.reso, kern.smooth = kern.smoothinZ, correct.env = F, R = Rin, run.silent.bak = run.silent, force.equal.sample=T, ncores=ncores)
	in.sb1.obs<-paste0("output.b1$",s.type)
    o.b1[[(i)]]<-(eval(parse(text=in.sb1.obs)))
	#measure and store background 2
	print(paste("Starting background tests 2->1 for distance:",round(ahe.i,0)))
	output.b2<-humboldt.background.test(zz, rep, sim.dir = 2, env.reso=env.reso, kern.smooth = kern.smoothinZ, correct.env = F, R = Rin, run.silent.bak = run.silent, force.equal.sample=T, ncores=ncores)
